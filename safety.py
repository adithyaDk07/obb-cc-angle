 ...
 ##############server side  
   ...
   #########serrver  
    ...
    def create_menu_bar(self):
        ...
        self.obb_menu.add_command(label="CUSTOM-OBB", command=self.load_custom_model)
...
##############additional button
        self.train_button = ctk.CTkButton(self.root, text="Train", command=self.train)
        self.train_button.place(relx=0.765, rely=0.82, relwidth=0.1, relheight=0.06)

        self.annotate_button = ctk.CTkButton(self.root, text="Annotate", command=self.annotate)
        self.annotate_button.place(relx=0.878, rely=0.82, relwidth=0.1, relheight=0.06)

        self.togg2_var = ctk.BooleanVar(value=False)
        self.togg2_switch = ctk.CTkSwitch(self.root, text="Enable Capture", variable=self.togg2_var, command=self.toggle_capture)
        self.togg2_switch.place(relx=0.77, rely=0.89)


        self.capture_button = ctk.CTkButton(self.root, text="Capture", command=self.capture_image)
        self.capture_button.place(relx=0.87, rely=0.89, relwidth=0.1, relheight=0.06)
...
##########
 ...
 #       self.capture_button.place(relx=0.77, rely=0.89, relwidth=0.2, relheight=0.06)
...
###############################33
    ...
    def capture_and_process_image(self):
        if self.captured_image is not None:
            ...
            self.terminal_frame.insert(tk.END, "No image captured for processing.\n")
...
#############OBB-UPDATES##################
    ...
    def _process_yolo_obb(self, image):
        # Dummy function for YOLO-OBB processing
        # In a real implementation, this would use the YOLO-OBB model to process the image
        # and return oriented bounding boxes
        self.terminal_frame.insert(tk.END, "Processing image with YOLO-OBB model\n")
        return np.array([])  # Placeholder return value

    def segregation_obb(self):
        # Dummy function for OBB-based segregation
        # This function would be similar to the segmentation-based segregation,
        # but would work with oriented bounding boxes instead of masks
        if self.captured_image_path is None:
            self.terminal_frame.insert(tk.END, "No image captured for OBB segregation.\n")
            return

        self.captured_image = cv2.imread(self.captured_image_path)
        if self.captured_image is None:
            self.terminal_frame.insert(tk.END, "Failed to load captured image for OBB segregation.\n")
            return

        self.clear_output()
        obb_results = self._process_yolo_obb(self.captured_image)

        # Process OBB results and display them
        # This would involve extracting centroids, orientations, and dimensions from OBBs
        # and sending this data to the robot for segregation

        self.terminal_frame.insert(tk.END, "OBB-based segregation completed.\n")

    def inspect_obb(self):
        # Dummy function for OBB-based inspection
        # This function would be similar to the segmentation-based inspection,
        # but would work with oriented bounding boxes instead of masks
        if self.captured_image_path is None:
            self.terminal_frame.insert(tk.END, "No image captured for OBB inspection.\n")
            return

        self.captured_image = cv2.imread(self.captured_image_path)
        if self.captured_image is None:
            self.terminal_frame.insert(tk.END, "Failed to load captured image for OBB inspection.\n")
            return

        self.clear_output()
        obb_results = self._process_yolo_obb(self.captured_image)

        # Process OBB results and perform inspection
        # This would involve comparing OBB dimensions with predefined limits
        # and classifying objects as OKAY or NOT OKAY

        self.terminal_frame.insert(tk.END, "OBB-based inspection completed.\n")
